      ...
      <div v-if="list && list.length > 0" class="card-container">
        <el-table
            ...
            <template slot-scope="scope">
              <el-form inline class="demo-table-expand" label-width="140px" label-position="left" >
                ...
                <template v-if="scope.row.service_type === 'REFUND' || scope.row.service_type === 'REFUND_RETURN'">
                  ...
                  <el-form-item label="发起人退款">
                    <span>{{ formatPrice(scope.row.sponsor_refund) }}元</span>
                  </el-form-item>
                  <el-form-item label="转发人退款">
                    <span>{{ formatPrice(scope.row.forwarder_refund) }}元</span>
                  </el-form-item>
                </template>
                <hr />
          ...
          <el-table-column
            ...
            label="SKU型号"
            width="180">
            <template slot-scope="scope">{{ scope.row.order.specification }}</template>
          </el-table-column>
          <el-table-column
            align="center"
            label="关联订单"
          ...
          <el-table-column
            align="center"
            label="订单状态">
            <template slot-scope="scope">{{ order_status[scope.row.order.status] }}</template>
          </el-table-column>
          <el-table-column
            align="center"
            v-if="$can('edit', 'Gift')"
            ...
            <template slot-scope="scope" v-if="scope.row.status === 'ADMIN_CONFIRMING'">
              ...
              <el-button type="text"
                         @click.stop="auditNo(scope.row.id)">不同意</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
export default {
  ...
  data () {
    return {
      ...
      loading: false,
      counts: {},
      status: constants.AFTERSALE_STATUS,
      service_type: {
        REPLENISHMENT: '补货',
        EXCHANGE: '换货',
        REFUND: '仅退款',
  ...
  methods: {
    ...
    query () {
      let that = this
      this.loading = true
      this.$axios.get('/gift_insider/gift_after_sale/?page_num=' + this.currentPage +
      ...
      this.$axios.get('/gift_insider/gift_after_sale/count/')
        .then(res => {
    ...
    gotoOrderDetail (id) {
      this.$router.push('/gift/orders/' + id)
    },
    handleSelect (key) {
      this.query_status = key
      this.getData()
    },
    doAudit (id, action, reason) {
      ...
      this.$axios.post('/gift_insider/gift_after_sale/' + id + '/confirm/', form).then(() => {
        that.query()
      })
    },
    auditNo (id) {
